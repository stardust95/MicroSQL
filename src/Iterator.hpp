...
public:
	...
	void Get (const char* attrName, char strAttr[]) const {
		...
		for ( int i = 0; i < count; i++ ) {
			if ( strcmp (attrs[i].attrName, attrName) == 0 ) {
				memcpy_s (strAttr, attrs[i].attrLength,
	...
	RecordIdentifier GetRid ( ) const { return rid; }
	void SetRid (RecordIdentifier r) { rid = r; }
...
public:
	TupleCmp (AttrType     sortKeyType,
			  ...
			  int     sortKeyOffset,
			  CompOp c)
		:c (c), p (sortKeyType, sortKeyLength, sortKeyOffset, c, NULL),
		sortKeyOffset (sortKeyOffset) {
	}
	// default - not the most sensible - here so I can make arrays
	TupleCmp ( )
		:c (EQ_OP), p (INT, 4, 0, c, NULL), sortKeyOffset (0) {
	}
